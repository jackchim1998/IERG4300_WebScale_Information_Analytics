...
for rand_index in [2]:
    ...
    with open("../data/test_images_eigenspace","r") as f_image:
	with open("../data/test_labels","r") as f_label:
	    line_read=0
		...
		for i in range(len(pt)):
			...
			print "cannot convert image to float"
                    	exit()
		#cal diff
		min_diff = float("inf")
		min_diff_cent = -1
	...
	for cent_index in range(len(centroids)):
	    ...
	    total_count+=centroids[cent_index].count
	    total_count_same_label+=label_count
	    f_out.write("centroid "+str(cent_index)+": label "+str(centroids[cent_index].final_label)+", count of same label "+str(label_count)+", count of all "+str(centroids[cent_index].count)+", accuracy "+str(float(label_count)/centroids[cent_index].count)+"\n")
	f_out.write("Total: count of same labels "+str(total_count_same_label)+", count of all "+str(total_count)+", accuracy "+str(float(total_count_same_label)/total_count)+"\n")
    centroids=[]
